...
int main()
{
  ...
  HANDLE_ERROR(cudaEventRecord(start, 0));//放在stream0里实际上就是放下立刻开始
  ...
  for (int i=0; i<FULL_DATA_SIZE; i+=2*N)  //order:两hdcopy一个kernel，一个dhcopy；unorder：两个order
  {
	...
	/*cudaStat = cudaMemcpyAsync(dev_a1, host_a+i+N, N*sizeof(int), cudaMemcpyHostToDevice, stream1);
...
// 		HANDLE_ERROR(cudaMemcpyAsync(host_c+i+N, dev_c1, N*sizeof(int), cudaMemcpyDeviceToHost, stream1));
  ...
  HANDLE_ERROR(cudaEventSynchronize(stop));//这种event的目的只是计算程序运行时间，
