...
/*__global__ void cal_k_kernel(long N, int *AD,int *d_OriR, float * d_OriV, double *d_K )
{		
	int offset;
	const int tid = blockDim.x * blockIdx.x+ threadIdx.x;
	for(offset = tid; offset<N; offset+=blockDim.x*gridDim.x )
		for (int j = d_OriR[offset]; j < d_OriR[offset+1]; j++) 
		d_K[offset]= (d_OriR[AD[offset]+1]-d_OriR[AD[offset]]);
}*/
...
bool Sub_Partition_GPU(int * OriR, int * OriC, int * R, int * C, float W2, int innerM, int * Result, int * Max_Result,int * AD)
{
...
*/
void Partition_GPU(int * R, int * C, float *V, double * k, int * Result)
{   
	int devID;
	...
	double W2;
	for (int i = 0;  < N; j++)
		W2 += k[i];			// The total Number connection in the network
	...
	while (Round <= Max_Result)	   //???????
	{
		...
		checkCudaErrors( cudaMemcpy( d_c, NewCol, sizeof(int) * (R[N]), cudaMemcpyHostToDevice)) ;
		checkCudaErrors( cudaMemcpy( d_Val, NewVal, sizeof(double) * (R[N]), cudaMemcpyHostToDevice)) ;
		...
		if (NumG)						 
		{
			...
			Start(1);
			if (NumG>25000)
				Issub = Sub_Partition_GPU(R,C,NewRow, NewCol, W2, innerM, Result, &Max_Result, Index_Result);
		//	else
...
int main(int argc, char * argv[])
{
	...
	for (int i = 0; i < FileNumber; i++)
	{
		...
		if (Vlength != Clength) cout<<"Weighted CSR file is damaged\n";
		...
		checkCudaErrors( cudaMalloc( (void**) &d_Val, sizeof(double) * (R[N]-R[0])));
	...
	// copy host memory to device
		...
		double * Ori_k = new double [N];
		memset((void *) Ori_k, 0, sizeof(double)*N);
		for (int i = 0; i < N; i++)
			for (int j = R[i]; j < R[i+1]; j++)
				Ori_k[i] += V[j];
		checkCudaErrors( cudaMemcpy( d_orik, Ori_k, sizeof(double) * N , cudaMemcpyHostToDevice)) ;
		...
		Partition_GPU(R, C, V, Ori_k, Modu_Result);
		Stop(0);
		...
		for (long long l = 0; l < Maslov_num; l++)
		{
			...
			Partition_GPU(R_dst, C_dst, V_dst, Modu_Result);
		}
		*/
		Stop(0);
