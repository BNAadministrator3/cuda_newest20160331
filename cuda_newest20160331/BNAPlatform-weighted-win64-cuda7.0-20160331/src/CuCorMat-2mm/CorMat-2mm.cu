...
//		cudaStat = cudaMemcpy(devBOLD, BOLD_t, sizeof(real__t) * L * N0, cudaMemcpyHostToDevice);
		...
		for (int kk = 0, ii = 0; ii < Num_Blocks; ii++)
		{
			for (int jj = ii; jj < Num_Blocks; jj++)
			{
				...
				else
				{
		            ...
		            { 
						if( out[i * Batch_size + j]>(*r_thresh-ep) && out[i * Batch_size + j]<=(1+ep) )
					    {
						  ...
						  out_t[j * Batch_size + i] = out[i * Batch_size + j]; //feishi~
				...
				time = clock()-time;
				cout<<"1.thresholding time:"<<time<<" ms"<<endl;
				time = clock();
				//2.generate local CSR  //do not forget to modify data survival	//feishi 
				int count = 0;
				long long Cindex = 0;
				...
				delete out_t;
				time = clock()-time;
				cout<<"2.generating locality time:"<<time<<" ms"<<endl;
				time = clock();
				...
				time = clock()-time;
				cout<<"3.push to global group time:"<<time<<" ms"<<endl;
				time = clock();
