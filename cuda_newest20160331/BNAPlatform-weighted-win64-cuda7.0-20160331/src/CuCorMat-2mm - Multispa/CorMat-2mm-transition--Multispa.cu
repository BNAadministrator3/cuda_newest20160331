...
real__t CorMat_spa2rth(string OutCor, real__t * BOLD, int N, int L, int Batch_size,real__t *s_thresh, int NumS, real__t* result, clock_t * aggregate)
{
	...
	uint__t *zeroAmount = new uint__t[NumS];
	for (int x = 0; x < NumS; x++)
	{
		uint__t amount = N * s_thresh[x] * (N-1)  / 100.0 ;///2.0 ;
		//amount += amount%2;
		zeroAmount[x] = N * N - amount;
	...
	for (int i = 0; i < L; i ++)
		...
		for (int x = 0; x < NumS; x++)
		{
			result[x] = width * ( thrust::upper_bound(histogram.begin(),histogram.end(),zeroAmount[x]) - histogram.begin() );
		}
