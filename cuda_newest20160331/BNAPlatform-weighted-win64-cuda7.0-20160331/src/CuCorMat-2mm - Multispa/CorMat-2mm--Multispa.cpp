...
int CorMat_gpu(string OutCor, real__t * BOLD, int N, int L, int Batch_size,real__t *result, int NumS,real__t width, real__t *s_thresh, clock_t* aggregrate);
real__t CorMat_spa2rth(string OutCor, real__t * BOLD, int N, int L, int Batch_size,real__t *s_thresh, int NumS, real__t *result,clock_t* aggregate);
...
int main(int argc, char * argv[])
{
	...
	for (int i = 0; i < FileNumber; i++)
	{
		...
		real__t *result = new real__t[NumS];
		CorMat_spa2rth(OutCor, BOLD, N, L, GPU_Batch_size,s_thresh, NumS, result, aggregate);
		/*for (int i = 0; i < NumS; i++)
		{
		cout<<result[i]<<endl;
		}*/
		////rthresh = 0.8;
		//sort 选出一个最小的 生成后再依次删除 生成文件 时间只算上一步加这一步correlation
		CorMat_gpu(OutCor, BOLD, N, L, CPU_Batch_size,result,NumS, interval(),s_thresh, aggregate);
		//sort sparsity //
