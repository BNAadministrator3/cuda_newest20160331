			...
			for (int jj = ii; jj < Num_Blocks; jj++)
			{
				...
				if(ii==jj)
				{
					...
					for (int i = 0; i < Batch_size; i ++)
					{
						...
						{
							if(out[i * Batch_size + j]>(result[mini]-ep)&&out[i * Batch_size + j]<=(1+ep)&&(i!=j))
							{
								tmp.column = j;
								tmp.column += ii * Batch_size;
								tmp.value = out[i*Batch_size+j];
								ColumnAndValue[ii*Batch_size+i].push_back(tmp);
								if (out[i*Batch_size+j]<(result[mini]+width)&&i<j)
								{
									bin.push_back(out[i*Batch_size+j]);
								}
							}
				...
				else
				{
					...
					for (int i = 0; i < Batch_size; i ++)
					{
						for (int j = 0; j < Batch_size; j++)
						{
							if( out[i * Batch_size + j]>(result[mini]-ep) && out[i * Batch_size + j]<=(1+ep) )
							{
								//1.push row
								tmp.column = j;
								...
								if (out[i * Batch_size + j]<(result[mini]+width))
								{
									bin.push_back(out[i * Batch_size + j]);
								}
							}
