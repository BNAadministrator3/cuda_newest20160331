...
real__t CorMat_spa2rth_stream(string OutCor, real__t * BOLD, int N, int L, int Batch_size,real__t *s_thresh, clock_t * aggregate)
{
	real__t * BOLD_t1, * BOLD_t2, * tempout;
	const int Num_Blocks = (N + Batch_size - 1) / Batch_size;
	//cout<<"debug: block number: "<<Num_Blocks<<endl;
	...
	for (int i = 0; i < L; i ++)
		...
		real__t *pinBOLDii, *pinBOLDjj,* devBOLDii,* devBOLDjj,* devCormat;// * devCormatLower, * devCormatPacked; //what the hell about mapped memory
		//uint__t *devhisto;
		//cudaStat = cudaMalloc ((void**)&devBOLD, sizeof(real__t) * L * N0) ;
		//cudaError_t err = cudaGetLastError();
        //printf("%s\n",cudaGetErrorString(err));
		cudaStat = cudaMallocHost (&pinBOLDii,sizeof(real__t)* L * Batch_size);
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		cudaStat = cudaMallocHost (&pinBOLDjj,sizeof(real__t)* L * Batch_size);
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		cudaStat = cudaMalloc ( (void**)&devBOLDii, sizeof(real__t) * L * Batch_size * num_ii) ;
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		cudaStat = cudaMalloc ( (void**)&devBOLDjj, sizeof(real__t) * L * Batch_size * num_jj) ;
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		...
		cudaStat = cudaMemcpy(devBOLDii, BOLD_t, sizeof(real__t) * L * Batch_size * num_ii, cudaMemcpyHostToDevice);
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		cudaStat = cudaMemcpy(devBOLDjj, BOLD_t, sizeof(real__t) * L * Batch_size * num_jj, cudaMemcpyHostToDevice);
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		cudaStream_t stream1;
		cudaStat = cudaStreamCreateWithFlags(&stream1,cudaStreamNonBlocking);//ar risk!!!!!!
		//cudaStat = cudaStreamCreate(&stream1);
		if (cudaStat != cudaSuccess) 
			return cudaStat;
		/*cudaEvent_t syn;
		cudaStat = cudaEventCreate(&syn);
		if (cudaStat != cudaSuccess) 
			return cudaStat;*/
		...
		{
			if (ii!=0&&ii<Num_Blocks+1-num_ii) //start and terminate condition
			{
				cudaStat = cudaMemcpyAsync(pinBOLDii, BOLD_t + L * Batch_size * ( ii+num_ii-1 ), sizeof(real__t) * L * Batch_size, cudaMemcpyHostToHost,stream1);
				if (cudaStat != cudaSuccess) 
					return cudaStat;
				cudaStat = cudaMemcpyAsync(devBOLDii + L * Batch_size * ( ii%num_ii-1<0?num_ii-1:ii%num_ii-1 ), pinBOLDii, sizeof(real__t) * L * Batch_size, cudaMemcpyHostToDevice,stream1);
				if (cudaStat != cudaSuccess) 
					return cudaStat;
			}
...
#else            
				time = clock();
				if (jj!=0&&xii<Num_Blocks)  
				{                                          
					//cudaStreamWaitEvent(stream1,syn,0);
					cudaStat = cudaMemcpyAsync(pinBOLDjj, BOLD_t + L * Batch_size * yjj, sizeof(real__t) * L * Batch_size, cudaMemcpyHostToHost,stream1);
					if (cudaStat != cudaSuccess) 
						return cudaStat;
					if ( yjj+1 >= Num_Blocks )
					{
					...
					cudaStat = cudaMemcpyAsync(devBOLDjj + L * Batch_size * ( ((ii * Num_Blocks - ii * (ii + 1) / 2 + jj)%num_jj)-1<0?num_jj-1:((ii * Num_Blocks - ii * (ii + 1) / 2 + jj)%num_jj)-1 ), pinBOLDjj, sizeof(real__t) * L * Batch_size, cudaMemcpyHostToDevice,stream1);
					if (cudaStat != cudaSuccess) 
						return cudaStat;
				}
				stat = cublasSgemm(handle, CUBLAS_OP_T,  CUBLAS_OP_N, Batch_size, Batch_size, L,  &alpha, devBOLDjj + ((ii * Num_Blocks - ii * (ii + 1) / 2 + jj)%num_jj) * Batch_size * L, L, devBOLDii + (ii%num_ii) * Batch_size * L, L, &beta, devCormat, Batch_size);
				if (stat != CUBLAS_STATUS_SUCCESS)
					return stat;
				/*if (ii!=jj)
				{
					cudaEventRecord(syn, 0);
				}*/
		...
		//another interface is needed to return the subscript		
			...
			*/
			//cudaFree (devBOLD); 
			delete []BOLD_t;
			cudaFreeHost(pinBOLDii);
			cudaFreeHost(pinBOLDjj);
			cudaFree(devBOLDii);
			cudaFree(devBOLDjj);
			cudaFree (devCormat);
			cudaStat = cudaStreamDestroy(stream1);
			if (stat != CUBLAS_STATUS_SUCCESS)
				return stat;
