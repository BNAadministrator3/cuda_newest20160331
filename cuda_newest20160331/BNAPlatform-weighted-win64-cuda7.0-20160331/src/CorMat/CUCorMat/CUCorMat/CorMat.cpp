...
/*	ifstream fin(string(argv[1]).append("\\mask.nii").c_str(), ios::binary);
	...
	if (argv[4][0] == 'n' || argv[4][0] == 'N' || argv[4][0] == 'b' || argv[4][0] == 'B' )
		for (int i = 0; i < FileNumber; i++)
		{
			...
			// set some parameters 
			int Batch_size = 1024 * 2;				// should not be smaller than 1024 !
			...
			time = clock() - time;
		//	cout<<"CPU correlation time: "<<time<<"ms"<<endl;
...
//					CorMat_cpu(Cormat_cpu, BOLD, N, L);
			...
			for (k = 0; k < NumS; k++)
			{
				...
				Outfilename = OutCor;
				Outfilename.append(sparsity).append("_weighted.csr");
			...
			//	fout.open(Outfilename.c_str(), ios::out);
				...
				fout.close();
				/*
				string OutCor = "E:\\severve-4mm-V.txt"; 
			    fout.open(OutCor.c_str());
				fout<<Rlength<<endl;
				for (long i = 0; i < Rlength; i++)
				{
					fout<<R[i]<<endl;
				}
				fout<<Clength<<endl;
				for (long long i = 0; i < Clength; i++)
				{
					fout<<C[i]<<endl;
				}
				fout<<Clength<<endl;
		...
		{
			// set some parameters 
			uint__t Batch_size = 1024 * 2;				// should not be smaller than 1024 !
			int Num_Blocks = (N + Batch_size - 1) / Batch_size;
			long long M2 = Num_Blocks * (Num_Blocks + 1) / 2;
			M2 *= Batch_size * Batch_size;
			real__t * Cormat_blocked = new real__t [M2];
