...
/**********函数功能：求对称、稀疏矩阵的特征向量中心度，参数依次分别为行偏移、列号、元素值、Ntemp*Ntemp*********************/
 ...
 double Lead_Vector_GPU(int *R, int *C,long Ntemp,double *v)  //beta到底用不用考虑 ，beta，d_u0,d_uu关系乱了！！
{
 ...
 //   double v_k;
	...
	{	  		
	    ...
	    cudaStat1= cudaMemcpy(v,d_u, sizeof(double) * Ntemp,  cudaMemcpyDeviceToHost) ;//这里是最终的输出了！这里求eigenvector
		if (cudaStat1 != cudaSuccess) 
      {
...
int main(int argc, char * argv[]){
	...
	for (int i = 0; i < FileNumber; i++)
	{
	   ...
	   // Parse file name
		...
		fout.write((char*)vf, sizeof(float) * N);  //保存矩阵的eigenvector centrality
		
		//fout.write((char*)&x, sizeof(float));           //保存矩阵的eigenvalue
		fout.close();
		//fout.open(X_cp_mas.c_str(),ios::trunc); //ios::trunc表示在打开文件前将文件清空,由于是写入,文件不存在则创建
