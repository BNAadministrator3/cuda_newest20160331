...
double CUBFW_Lp(int *row, int *col, int numVertices, int numEdges)
{
	...
	init_block(costmatPaded, rowPaded, col, numVerticesPaded, sizeBlock);//(symmetric and) sparse graph -> full dense blocked graph  numVertices? or numVerticesPaded?
#ifndef paper
		...
		for (int j = 0; j < cntBlock; j++)
		{
			for (int ii = 0; ii < sizeBlock; ii++)
			{
				for (int jj = 0; jj < sizeBlock; jj++)
				{
					...
					{
						if (costmatPaded[index]<1 && costmatPaded[index]>numVertices)
