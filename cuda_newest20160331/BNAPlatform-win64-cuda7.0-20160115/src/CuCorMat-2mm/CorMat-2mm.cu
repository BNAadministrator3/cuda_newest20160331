			...
			//	out = Cormat + (long long) kk * Batch_size * Batch_size;
				...
				if(ii==jj)
				{
				  for (int i = 0; i < Batch_size; i ++)
		            ...
		            { 
						if(out[i * Batch_size + j]>*r_thresh&&out[i * Batch_size + j]<=1&&(i!=j))
					    {

						  out_t[i * Batch_size + j] = 1;
				...
				else
				{
					...
					for (int j = 0; j < Batch_size; j++)
		            { 
						if(out[i * Batch_size + j]>*r_thresh&&out[i * Batch_size + j]<=1)
					  {
						 
						  out_t[j * Batch_size + i] = 1;
						  out[i * Batch_size + j] = 1;
